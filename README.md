# podinfo
This fork enhances the original podinfo microservice with a custom CI/CD workflow:

* Implemented a complete CI/CD pipeline using Jenkins for continuous integration and Flux for continuous deployment
* Modified the UI to personalize the application interface
* Added a Jenkinsfile to automate Docker image builds and pushes to Docker Hub
* Containerized the application using Docker
* Developed Kubernetes manifests and a Helm chart for deployment
* Configured Flux CD to automatically deploy from the Git repository to a local Kubernetes cluster using kind
* Adopted GitOps practices by using the repository as the single source of truth for deployments
